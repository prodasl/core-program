...
It is a cryptographic framework that allows one party to prove the truth of a statement to another party without revealing any additional information beyond the validity of the statement.
2. What is input, witness, circuit, and proof? 
**Input:** Data provided to the proof system to validate a claim.
Example: A transaction amount in a zkSNARK proof.

**Witness:** Additional secret data needed to support the proof.
Example: The private key used to generate a signature.

**Circuit:** A mathematical representation of the statement being proven.
Example: A circuit verifying that the sum of inputs equals a specific value.

**Proof:** Evidence generated by the proof system to convince a verifier of the statement's truth.
Example: A zkSNARK proof that confirms a transaction is valid without revealing details.

3. How does program convert to proof?
Convert program to a bunch of constraints to be able to get the input data and evaluate if the data complains with the constraints.
